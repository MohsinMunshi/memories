...
const Post = ({post, setCurrentId }) => {
    ...
    return (
      ...
      <CardActions className={classes.cardActions}>
        <Button style={{fontSize: "10px"}} size="small" color="primary" disabled={!user?.result} onClick={() => dispatch(likePost(post._id))}>
          <Likes />
        </Button>
        {(user?.result?.googleId === post?.creator || user?.result?._id === post?.creator) && (
          <Button size="small" color="secondary" onClick={() => dispatch(deletePost(post._id))}>
            <DeleteIcon  fontSize="small" /> &nbsp; <span style={{fontSize: "10px"}}>Delete</span>
          </Button>
        )}
      </CardActions>
    </Card>
